...
| FR-008 session watchlists, zones, narrow safe text masks | PARTIAL | Session-only private-word UI, English PP-OCRv5 source/package wiring, and configured zones exist. The new OCR runtime has not been device-run or accuracy-evaluated; BIV was 0/16. |
...
| SC-009 ≥90% reliable text cases retain outside content | UNMET | Mapper/evaluator tests encode the metric. The new English PP-OCRv5 private-word implementation is source/package verified only; it has no device or accuracy result and HOLDOUT remains sealed. |
